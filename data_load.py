...
class DataFlowAPI(RNGDataFlow):
    ...
    def __call__(self, n_prefetch=1000, n_thread=1):
        ...
        df = PrefetchData(df, n_prefetch, n_thread)
        return df
class Net1DataFlowAPI(DataFlowAPI):

    def get_data(self):
        while True:
            #wav_file = random.choice(self.wav_files)
            yield get_mfccs_and_phones(wav_file=self.wav_file)
class Net2DataFlowAPI(DataFlowAPI):

    def get_data(self):
        while True:
            #wav_file = random.choice(self.wav_files)
            yield get_mfccs_and_spectrogram(self.wav_file)
