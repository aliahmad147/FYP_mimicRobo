...
from data_load import Net2DataFlowAPI
...
def do_convert(ckpt, case2, logdir1, logdir2, wavs):
    # Load graph
    model = Net2()
    hp.set_hparam_yaml(case2)
    df = Net2DataFlowAPI(wavs, hp.convert.batch_size)

    ckpt1 = tf.train.latest_checkpoint(logdir1)
    ckpt2 = '{}/{}'.format(logdir2, ckpt) if ckpt else tf.train.latest_checkpoint(logdir2)
    session_inits = []
    if ckpt2:
    ...
    audio, y_audio, _ = convert(predictor, df)
...
def get_arguments():
    ...
    parser.add_argument('wavs', type=str, help='wav file')
...
if __name__ == '__main__':
    ...
    do_convert(args.ckpt, args.case2, logdir1=logdir_train1, logdir2=logdir_train2, wavs=args.wavs)
